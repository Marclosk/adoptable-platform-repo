...
class ContactViewTest(APITestCase):
    ...
    def test_success_creates_record_and_sends_email(self):
        ...
        # parcheamos EmailMessage.send para no enviar realmente
    ...
    def test_email_send_error_returns_500(self):
        """Si falla al enviar email → 500 + mensaje de error"""
        ...
        self.assertEqual(resp.status_code, status.HTTP_500_INTERNAL_SERVER_ERROR)
        self.assertEqual(resp.data.get('error'), 'Error al enviar el correo.')
        # aunque leyó bien, no importa que quede el registro en BD según tu vista
