  ...
  const fetchSuggestions = async (val: string) => {
    ...
    try {
      const [aRes, uRes] = await Promise.all([
        axios.get(`/api/animals/?search=${encodeURIComponent(val)}`),
        axios.get(`/users/?search=${encodeURIComponent(val)}`),
      ]);
      const animals: Suggestion[] = aRes.data
        .slice(0, MAX_PER_TAB)
        .map((a: any) => ({
          id: a.id,
          label: a.name,
      ...
      const users: Suggestion[] = uRes.data
        .slice(0, MAX_PER_TAB)
        .map((u: any) => ({
          id: u.id,
          label: u.username,
